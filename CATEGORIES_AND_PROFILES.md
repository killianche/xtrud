...
## CURRENT / TARGET — override 2026-08-23

**CURRENT:** исполняемый scope-фильтр
[`src/lib/product-scope.ts`](src/lib/product-scope.ts) показывает только
`construction` и `home-services`. Migration chain ожидает те же две L1 после
`0068`, но фактический состав production БД нельзя утверждать до live read-only
inventory.

**TARGET:** универсальная доска использует 10 пользовательских L1 из дерева
ниже и отдельный скрытый fallback для «не знаю категорию». TARGET не включён и
не должен появляться в UI до поэтапного feature rollout. Единый контракт scope,
unknown path, category flags, matching и backward compatibility —
[`docs/UNIVERSAL_TASK_BOARD.md`](docs/UNIVERSAL_TASK_BOARD.md).

Дерево 10 L1 и поздние концептуальные схемы ниже — источник seed-контента и
история проектирования, а не снимок runtime schema. При противоречии с
исполняемым кодом, generated types или новым universal contract они не
описывают CURRENT.
...
Изначально каталог содержал 10 L1. В Git migration chain файл [`0068_drop_out_of_scope_categories.sql`](supabase/migrations/0068_drop_out_of_scope_categories.sql) удаляет 8 L1 (Авто, Перевозки, Бьюти, Образование, События, Бизнес, IT, Личный сервис) вместе со связанными demo/dev-данными. Ожидаемое после этой миграции состояние — **2 L1 раздела:**
...
**История сужения:** на dev/demo-окружении out-of-scope категории создавали путаницу, а seed-мастера в этих L1 становились orphaned. Для универсальной доски категории возвращаются только новой forward-only additive migration; production backup не используется как способ выборочно «достать» seed.
...
**Расширение scope:** выполнять только по rollout из
[`docs/UNIVERSAL_TASK_BOARD.md`](docs/UNIVERSAL_TASK_BOARD.md): live inventory +
backup/restore rehearsal → P0 security → forward-only taxonomy/schema →
совместимый клиент → отдельная activation migration для feature flags. Restore
migration оставляет новый scope fail-closed. Одного re-seed или изменения
`IN_SCOPE_L1_IDS` недостаточно.

---

## TL;DR

- **CURRENT:** клиент показывает 2 L1 (`construction` + `home-services`); точные production counts требуют live inventory.
- **TARGET:** 10 пользовательских L1 + внутренний hidden fallback; L2/L3 включаются по отдельным category flags, а не одним глобальным списком.
...
1. **L1** — крупная сфера жизни. В TARGET это вход в масштабируемый search-first picker; 10 L1 нельзя показывать в CURRENT до rollout.
...
4. Каждый L3 имеет стабильный **id**, отображаемое имя, icon metadata, типичный
   чек/срок и признаки допуска. Emoji не используется как UI-иконка; старые
   Lucide-имена в seed считаются legacy metadata и отображаются через актуальный
   Phosphor/Iconify mapping.
...
Актуальные CURRENT-поля подтверждаются
[`src/features/orders/order-schema.ts`](src/features/orders/order-schema.ts) и
generated database types. Целевой additive контракт —
[`docs/UNIVERSAL_TASK_BOARD.md`](docs/UNIVERSAL_TASK_BOARD.md) § 4.

Критические границы: до 5 фото; один `budget_value`, не диапазон; `by_date`
хранит `preferred_date`; номер клиента скрыт; `work_mode` разделяет выезд и
remote; unknown-category использует внутренний fallback. In-app contact mode,
точный адрес «выбранному» мастеру, `picked_master_id` и lifecycle-статусы не
являются частью пользовательской модели.

### 3.2. Category-specific поля заказа

Ниже сохранены исторические идеи category-specific полей. Они не должны
автоматически превращать одноэкранную форму универсального задания в длинный
визард. Обязательность и допуск регулируемой услуги определяются category
contract и серверной валидацией.
...
|---|---|---|
| Простота входа | ✅ один login и одна учётная запись | ❌ две регистрации |
...
- Настройки
- Правила безопасности и модерации
...
Рейтинг есть только у мастера: клиент оценивает мастера после фактической
работы. Мастер не оценивает клиента. Исторические поля/идеи client rating не
возвращать без нового явного продуктового решения.

**Публичная карточка профиля:**
- Если человек **только клиент** — публичной карточки каталога нет.
...
**Решение:**
- Клиентский запрос «не знаю категорию» использует внутренний hidden fallback и
  сохраняет `requested_service_text`; fallback не отображается как публичная
  профессия мастера. Контракт — `docs/UNIVERSAL_TASK_BOARD.md`.
- Повторяющиеся unknown-запросы анализируются без PII и дают сигнал создать или
  включить реальную L2 после модерации и проверки supply.
...
**Решение:**
- L3 помогает поиску, но не обязателен для публикации универсального задания.
- Пользователь может подтвердить предложенную L2/L3 или выбрать «Не знаю
  категорию»; исходный `requested_service_text` сохраняется. Автоматика не
  подменяет категорию без audit trail и не блокирует публикацию только из-за
  несовпадения текста.
- «Для меня» использует подтверждённую/доверенную L2 и eligibility; fallback до
  классификации остаётся в «Все» без массового уведомления.
...
│  + additive universal fields: см.                      │
│    docs/UNIVERSAL_TASK_BOARD.md § 4                     │
└─────────────────────────────────────────────────────────┘
...
│  price_value, price_kind, lead_time, message text,      │
...
- Блокировка и report/moderation должны фильтровать list/detail/RPC/Realtime и
  прямые URL; полный контракт — `docs/UNIVERSAL_TASK_BOARD.md` § 7.
