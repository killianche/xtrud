...
## Решение владельца 2026-09-06 — заголовки iOS, разделы каталога, один набор иконок

> DECISION по серии скриншотов сборок 26–28. Референс продукта — **Thumbtack**
> (DECISION владельца 2026-09-05: «Thumbtack — наш референс, запиши»). Второй
> ориентир — системные приложения iOS 26.

### Заголовок экрана — как в системных приложениях iOS

Компоненты: `src/components/ui/LargeTitle.tsx` (`useLargeTitle`,
`LargeTitleBar`, `LargeTitleBlock`). Метрики Apple: крупный заголовок 34/41
(`text-ios-large-title`), компактный 17/22 (`text-ios-title`).

- Крупный заголовок живёт **в содержимом** и уезжает при прокрутке; в
  закреплённой строке навигации в этот момент проявляется компактный.
- Строка навигации — 44 pt, в материале Liquid Glass; под ней может стоять
  панель (поиск, фильтры, сегменты), закреплённая вместе с ней.
- Действия справа — текстом в акценте или иконкой в круге (как «+» и «⋯» у
  Apple). Собственных пилюль с рамкой в шапке больше нет.
- Применяется на всех вкладках: Задания, Специалисты, Мои задания.

### Отступ от чёлки — на каждом экране, и это проверяет тест

`src/components/ui/screen-top-inset-contract.test.ts` проходит по всем
маршрутам в `app/` и требует, чтобы экран либо сам учитывал `insets.top`,
либо использовал компонент, который делает это за него. Причина — шторки
выбора рисовали заголовок под часами. Исключение одно: главная, у которой
фото-герой намеренно уходит под статус-бар.

### Каталог: разделы на главной, категории внутри

Семь разделов (`categories_l1`): Ремонт и отделка · Сантехника и электрика ·
Строительство и участок · Дом и быт · Мебель и интерьер · Техника и
безопасность · Мастер на час и переезды. На главной показываются разделы;
тап ведёт на «Специалисты» с фильтром по разделу, где категорию можно
уточнить. Какие разделы активны, решает **база** (`is_active`), списка в коде
нет — разошедшийся список однажды спрятал половину каталога.

### Один набор иконок — Phosphor

Цветные иконки с чужого CDN (Twemoji, Fluent) и линейные Lucide убраны. Все
иконки категорий и разделов — Phosphor, линейные, одной толщины, в цвете
токена. Как у Thumbtack. Имена иконок лежат в базе и сверены со списком
пакета (миграция 0157).

### Экран категории → «Специалисты»

Маршрут `/category/[id]` — переход на «Специалисты» с фильтром. На самом
экране: поиск, чипы «Категория · Город · Сортировка», всё считает один RPC
(`search_masters`, миграция 0158).

### Вход должен быть виден

Для гостя на главной вместо бледного значка — пилюля «Войти». Человек должен
понимать, где у него аккаунт.

## Решение владельца 2026-09-06 (вечер) — путь гостя, контакты в задании, создание задания с нуля

> DECISION владельца: «если я не зарегистрирован, кнопка „Откликнуться“ всё
> равно должна быть, по нажатию — регистрация и возврат; при создании задания
> — телефон и WhatsApp по желанию, можно не тот, что в аккаунте; создание
> задания переделать с нуля, как у TaskRabbit и iOS».

### Гость видит то же, что и вошедший

Правило Apple (HIG, Managing accounts): дать пользоваться приложением до
аккаунта и просить вход в момент, когда он действительно нужен, объяснив
зачем. У нас такой момент один — отклик.

- На открытом задании гость видит ту же кнопку «Откликнуться». Тап открывает
  шторку `RespondAuthSheet`: «Заказчику важно знать, кто пишет», две кнопки —
  «Создать аккаунт» (главная) и «У меня уже есть аккаунт».
- Возврат — через существующий безопасный return-intent (`/orders/<id>` в
  allowlist). После входа или регистрации человек оказывается на том же
  задании, форма отклика уже на экране; экран задания снимает intent сам.
- Публикация задания по-прежнему просит вход на последнем шаге; черновик
  переживает вход.
- На главной для гостя — пилюля «Войти» вместо бледного значка.

### Контакты в задании — по желанию заказчика

Колонки `orders.contact_phone`, `orders.whatsapp_phone` (миграция 0159, с
колоночными грантами — у `authenticated` на `orders` только они). Поля на
последнем шаге создания и в редактировании; телефон подставляется из аккаунта
один раз и только в пустое поле. Оставленные контакты видят откликающиеся —
кнопки «Позвонить» и «WhatsApp» под карточкой заказчика. Пусто — связь через
отклик, как раньше.

### Создание задания — один вопрос на экран

Изучено: последовательность TaskRabbit (категория → где → когда → детали →
проверка → подтверждение) и правила Apple «Entering data» (выбор вместо
ввода, разумные значения по умолчанию, «Далее» только когда шаг заполнен,
проверка сразу, спрашивать только нужное).

Шесть шагов, каждый — настоящий маршрут Stack (`/orders/new`, `/new/details`,
`/new/where`, `/new/when`, `/new/budget`, `/new/review`): «назад» и свайп от
края — один POP, черновик живёт в store.

1. Что нужно сделать? — поиск, подсказки, категории (без автоприсвоения).
2. Расскажите подробнее — описание с подсказкой под категорию и фото.
3. Где это? — город/район.
4. Когда нужно? — сроки чипами, «К дате» через системный календарь.
5. Какой бюджет? — вид и сумма, либо договорная.
6. Проверьте задание — сводка, каждая строка ведёт на свой шаг; имя и
   контакты; «Опубликовать бесплатно».

Шапка шага: «назад», компактный заголовок «Новое задание», полоса прогресса.
Вопрос — крупно в содержимом, подсказка под ним. Главная кнопка — внизу, в
зоне большого пальца, выключена, пока шаг не заполнен.

## Решение владельца 2026-09-06 (ночь) — фильтры как в iOS, два цвета ролей, скорость на 1000 заданий

> DECISION владельца: «фильтры при поиске заданий и специалистов — как в
> последнем iOS: приёмы, иконки, шрифты»; «„Как мастер“ — чёрный акцент,
> чтобы человек понимал контекст»; «на главной — „Категории специалистов“»;
> «подключи тестировщика: чтобы не зависало даже при 1000 заданий».

### Фильтры — строка чипов под заголовком и системные шторки

Источник — WWDC26 «Design intuitive search experiences»: лёгкая фильтрация
живёт строкой под полем поиска (scope bar / filter row), подробный выбор — в
шторке; показывать только те фильтры, что уместны здесь.

- ⚠️ 2026-09-08: чипы фильтров в интерфейсе не используются — фильтры
  выбираются круглыми кнопками в строке заголовка и системными шторками.
  Компонент оставлен на случай возврата.
- Общий `FilterChip` (`src/components/ui/FilterChip.tsx`): капсула 44 pt,
  текст 16, иконка 17, стрелка вниз («открывает выбор»); активный чип
  показывает выбранное («Сантехника +2»), а не название фильтра.
- Задания: чипы «Категория» и «Место» под заголовком «Задания» + «Сбросить».
  Отдельный экран «Фильтры» удалён — было два механизма для одного дела.
- Выбор — `formSheet` на `PickerSheetPage`: одиночный с галочкой, множественный
  (категории) с кружками-галочками и кнопкой «Готово · N»; коммит только по
  «Готово», жест закрытия ничего не меняет.
- Специалисты — те же чипы и шторки; список догружается страницами по 30.

### Две роли — два цвета

Сегменты «Мои задания»: «Как клиент» в фирменном акценте, «Как мастер» —
чёрный (`primary`/`on-primary`). Переключение — с системной хаптикой выбора.

### Скорость на 1 000+ заданий (по разбору QA и технической роли)

Сервер (миграции 0161–0164, применены и проверены на живой базе):
индексы под порядок ленты и фильтры; дневной лимит откликов считает только
сегодняшние строки; политика чтения откликов без вложенного RLS; рассылка
«новое задание» ушла из транзакции публикации в очередь с воркером pg_cron;
живые обновления — одна личная подписка на `notifications` (в публикации
realtime раньше не было ни одной таблицы — старые подписки молчали).

Клиент: `useWatch` по нужным полям вместо `watch()` всей формы; запись
черновика в Keychain с паузой 400 мс, а не на каждую букву; лента берёт только
id откликнутых заданий, а не все отклики с JOIN; стабильные зависимости
realtime-подписки. Нагрузочный тест с 1 006 заданиями (в откатываемой
транзакции): первая страница ленты 6,7 мс, глубокая — 0,8 мс.

## Решение владельца 2026-09-06 (вечер) — как у Apple: шторка выбора, плавающий «+», заголовок первым, логотип на сплэше

> DECISION владельца по скриншотам сборки 32: «„+“ — плавающим снизу справа,
> как у iOS»; «заголовок „Категория“ маленький — как на iOS; выбранное
> незаметно; непонятно, что „весь раздел“ — раздел»; «заголовок „Задания“ —
> в самом верху, где пустое место, фильтры под ним»; «на сплэше сетка и
> кружочки — нужен логотип»; «ориентируемся на последний iOS: иконки, размеры
> текста, шрифты — делай то же самое».

### Что взято у Apple (HIG, iOS 26)

| Элемент | У Apple | У нас |
|---|---|---|
| Служебные иконки | SF Symbols: chevron.left, xmark.circle.fill, checkmark, plus, chevron.down, magnifyingglass | `SystemIcon` — SF Symbol на iOS, Phosphor того же смысла на Android/web |
| Шторка выбора | Title 1 28/bold слева, xmark.circle.fill 30 справа; inset grouped список, заголовки групп Footnote 13 заглавными; строка 44+ с плиткой иконки 29×29; выбранное — checkmark в цвете акцента | `PickerSheetPage` с `sections`; выбранная строка дополнительно на `accent-soft` — владелец просил заметнее |
| Раздел целиком | группа = раздел | первая строка группы «Весь раздел» с плиткой, залитой акцентом, и полужирным текстом |
| Главное действие списка | круглая стеклянная кнопка снизу справа над меню (Заметки, Почта) | `FloatingActionButton` 56 pt, Liquid Glass; без стекла — акцентная заливка с тенью |
| Порядок в шапке | large title → сегменты/фильтры под ним (Фитнес, Здоровье) | «Задания» и «Мои задания»: заголовок первым, чипы/сегменты в начале списка, уезжают со списком; в закреплённой строке — компактный заголовок |
| Шкала текста | Large Title 34, Title 1 28, Title 2 22, Body 17, Callout 16, Subheadline 15, Footnote 13 | токены `text-ios-*` в `tailwind.config.ts` |
| Сплэш | простой фон + знак | знак xtrud в фирменном цвете на белом, белый на чёрном (`splash-icon*.png` из `xtrud-logo.svg`) |

Правило про иконки уточнено в `docs/IOS_FOUNDATION.md` §7: служебные —
SF Symbols на iOS через `SystemIcon`, содержимое и категории — Phosphor.


## Решение владельца 2026-09-06 (ночь) — конструктор задания с нуля

> DECISION: «изучи, как Apple, TaskRabbit и подобные делают форму заявки, и
> сделай нашу с нуля, не ориентируясь на старую, в стиле Apple iOS Liquid
> Glass». Исследование и правила — `docs/TASK_COMPOSER.md`.

Семь экранов-вопросов на маршрутах Stack: «Что нужно сделать?» → «Расскажите
подробнее» → «Где?» → «Когда?» → «Какой бюджет?» → «Как связаться?» →
«Проверьте задание». Вопрос — Title 1, ответы — inset grouped строки с
галочкой, поля — по одному, с проверкой после ухода из поля. Главное действие
— выпуклая капсула Liquid Glass внизу; клавиатура её поднимает. Экран
проверки — строки-ответы, каждая ведёт на свой шаг. Старые `OrderFormBody`,
`TaskIntentStep`, три шторки-пикера и календарь удалены.


## Решение владельца 2026-09-07 — строка навигации без плашки, конструктор крупнее

> DECISION: «откуда белая плашка с кнопкой назад? сделать как в Apple iOS 26
> Liquid Glass»; «в начале — выбор категории; название — чтобы понятно, что
> вписывать; поля и кнопки больше; отмена выхода на любом шаге».

- Строка навигации в покое не имеет фона: содержимое доходит до верха,
  «назад» и действия — круглые стеклянные кнопки 44 pt. Фон в материале и
  компактный заголовок проявляются только когда содержимое уезжает под
  строку (та же прозрачность, что у заголовка).
- Конструктор задания: категория — первый вопрос (inset grouped по разделам
  с поиском), название — второй, с подписью «Название задания», примером и
  подсказкой; «Закрыть» справа на каждом шаге; строки 56 pt, поля 60 pt
  (ввод 24/18 pt), главная капсула 56 pt.


## Решения владельца 2026-09-07 (день) — iOS 26 везде: фильтры, поиск, аккаунт, специалист

> DECISION: «фильтры, пилюли, строки поиска — как в iOS 26 Liquid Glass,
> редизайн с нуля»; «специалисты: сначала категории, потом люди; свайп назад
> должен работать»; «на главной — имя и аватар, тап открывает аккаунт;
> плавающий „+“ при прокрутке»; «профиль специалиста: категории, о себе, фото
> работ, контакты — и всё; дальше отзывы»; «пустое место над заголовком —
> убрать».

- Таблица соответствий iOS 26 — `docs/IOS_FOUNDATION.md` §0.
- Поиск — стеклянная капсула (`SearchField`); фильтры — круглые кнопки
  (`NavCircleButton`) и системные шторки выбора;
  шторка категории одна на ленту и специалистов: «Все категории», разделы с
...
- «Специалисты» — список категорий; список людей — экран стека
  `/specialists/section` (свайп назад).
- Главная: пилюля «аватар + имя» → `/account`; плавающий «+» после
  прокрутки шапки.
- Аккаунт (`/account`) и «Я специалист» (`/profile/specialist`) — экраны в
  стиле Настроек; публичная страница специалиста (`/master/[id]`) — по
  карточке исполнителя TaskRabbit: имя и рейтинг, чем занимается, о себе,
  фото работ, услуги, отзывы, стеклянные «Позвонить» / «WhatsApp».
- Сервер: специалист виден в каталоге с первой категорией (0169).
