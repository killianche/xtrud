...
**Sprint 9 закрыт.** 9.1–9.3 готовы: full master profile edit, public /client/[id], outcome tracking modal. Клиент через 3 дня после accept получает modal с тремя опциями (Всё сделано / Не договорились / Спросить позже) — закрытие orphan-orders при контакте offline. Симметрия master ↔ client reputation полностью покрыта UI.

**Backlog после Sprint 9:** live-тест на dev-build (когда eas init будет запущен); admin-tool для category-covers; real OTP / Telegram Login (снимет advisor anonymous warnings); test runner (Vitest + Maestro); database-уровень CHECK на edit-status order.
