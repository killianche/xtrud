...
**Sprint 12 закрыт.** 12.1 lightbox, 12.2 chats unread badge, 12.3 orders unread badge, 12.4 master feed infinite scroll, 12.5 pinch+pan+double-tap в lightbox.

**База:** 22 миграции, 15 таблиц с RLS + 3 Storage bucket, 7 RPC, 12 trigger functions, 17 enums, 1 edge function.

**Backlog после Sprint 12:** master-side badge на «Заказы» (новые ордера в feed); live-тест dev-build; admin-tool для category-covers; real OTP / Telegram Login; swipe-between-photos в lightbox.
...
- [x] **2026-05-12** — **12.5** Pinch + double-tap в lightbox (commit pending): добавлен `GestureHandlerRootView` в `app/_layout` для всего приложения. `PortfolioLightbox` оборачивает Image в `GestureDetector` с `Gesture.Simultaneous(pinch, pan, Gesture.Exclusive(doubleTap, singleTap))`. Pinch 1×–4×, pan активен только при scale > 1×, double-tap toggle 1× ↔ 2.5×, single-tap закрывает только при scale=1× (чтобы не закрывать zoom-юзеру). При смене index zoom + pan плавно сбрасываются через `withTiming(200ms)`. Использованы Reanimated 4 `useSharedValue` + `useAnimatedStyle`.
