...
**Sprint 9 в работе.** Закрыты 9.1 (master edit) + 9.2 (client public view). Теперь любой участник тапнув на имя мастера/клиента в OrderInfoBlock или в шапке чата открывает полную карточку. Master `/master/[id]` — портфолио и отзывы. Client `/client/[id]` — рейтинг + «N заказов закрыто» + отзывы мастеров. Симметрия master ↔ client reputation готова.
