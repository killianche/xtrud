...
- FACT сверка уведомлений (0185): «Нашёл исполнителя» больше не шлёт
  выбранному «отменил»; истёкшее задание — уведомление и клиенту;
  скрытый отклик — «Отклик отклонён» специалисту; скрытие/возврат
  профиля админом — уведомление специалисту. Рассылка «Новая заявка»
  проверена прогоном с откатом — работает.
- DECISION владельца 2026-09-11: каждый аккаунт сразу специалист, «Стать
  специалистом» убрано. FACT (0186): профиль создаётся при регистрации,
  скрыт до первой категории; статус нельзя снять обновлением; телефон
  отдаётся только специалисту из каталога.
- FACT экран задания (сборка 80): «Заказчик» и отклики — карточки по
  стандарту DESIGN.md, «Позвонить» в акценте (без чёрных кнопок), контакты
  в откликах видны сразу; «Ваш отклик» — отдельный блок со статусом;
  число откликов из «Деталей» убрано.
- FACT поля конструктора (сборка 81): рамка видна без фокуса (в тёмной
  теме поле сливалось с фоном), нажатие в любое место плитки ставит курсор.
- DECISION владельца 2026-09-11: специалист каждый, но в каталоге и в том,
  что к нему относится, — только с категорией; откликаться можно без неё.
  FACT (0187, сборка 81): «Принимаю заказы» без категории не включается ни
  в приложении, ни на сервере; у статуса — ⓘ со шторкой «Статусы профиля».
- FACT закрыты отложенные пункты: ссылка специалиста (0188), политика на
  сайте и в приложении приведена к правде (вход по телефону, восстановление
  через поддержку, прямой APNs, без Supabase и сторонних CDN), файлы S3 в
  ночной копии (`deploy/mirror-s3-to-disk.py`), `.env.example`, удалён
  мёртвый код.
- DECISION владельца 2026-09-12: отзыв — один в три дня (0191). В базе
  было 7 дней, на экране написано «30 дней»; теперь одно число везде.
- FACT поддержка — WhatsApp +7 928 920-40-29 (DECISION владельца
  2026-09-11): `t.me/xtrud_support` не существовал, адрес стоял в 8 местах.
- FACT админка (0190): смена пароля из панели падала на ограничении журнала
  (`set_password` не был разрешён) — исправлено; добавлена смена номера
  входа для возврата аккаунта, с отзывом сессий и записью в журнал.
- FACT живые обновления (0189 + сервер + сборка 83): новое уведомление →
  pg_notify → `GET /v2/events` → приложение обновляет счётчики сразу;
  опрос раз в 20 с — запасной путь, пока канала нет. Проверено через nginx.
