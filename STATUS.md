...
## 📍 Снимок: где мы сейчас (2026-05-29)

- **Модель:** доска объявлений (classifieds) — клиент создаёт заказ → мастера откликаются → клиент сам звонит/пишет в WhatsApp. Без чата, без lifecycle сделки, без спора, без рейтинга клиента, без email, без фамилии. Полное описание — CLAUDE.md «🧭 Актуальная модель продукта».
- **Сайт:** https://xtrud.alanbani.ru/ — **выкачено 2026-05-29** (последний батч ниже). Demo-вход включён (превью-площадка).
- **Стек:** Expo SDK 54 + Expo Router + Supabase + NativeWind + TanStack Query + Phosphor.
- **Свежее (2026-05-29):** срок «К дате» в заказе, редизайн окон выбора + edit-master, фавикон, фикс фото заказа, обжалование отзыва мастером. Всё в проде.
- **Следующее:** очередь пуста. Перед публичным запуском — выключить demo-вход (`EXPO_PUBLIC_ENABLE_DEMO`) и вернуть `acceptedTerms=false` в `phone.tsx`.
