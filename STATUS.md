...
- ⚠️ ИНЦИДЕНТ: резервная копия `xtrud-api.bak-2026-09-14-xff` лежит в `/etc/nginx/sites-enabled/` — `nginx -t` падает; сайт и API работают, но перезапуск nginx не пройдёт. Убрать может только владелец (команда в TASKS 106).
