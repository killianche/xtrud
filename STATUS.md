...
## Android и RuStore, единые статусы, ревью безопасности — 2026-09-14

- FACT Android: сборка на GitHub Actions (`android.yml`), ключ подписи RSA 4096 (VDS + секреты), APK проверен эмулятором Android 14 — запуск без падений, все вкладки меню видны; SDK RuStore Push в `vendor/` с правкой под RN 0.86. Первая версия в RuStore загружается владельцем вручную (API требует активную версию); `SYSTEM_ALERT_WINDOW` убран.
- FACT push RuStore: проект и сервисный токен проверены живым запросом; отправитель в `server/src/push/rustore.ts`. Не включён: нужна выкладка сервера и 0193.
- FACT статусы заданий: единый `orderStatusView` + `StatusPill`, без обводок, «Архив» ниже активных, счётчик активных, контраст AA (designer → implementer → qa → reviewer «принято»). UNKNOWN: вид на iPhone.
- FACT безопасность (xtrud-security): лимиты по IP обходились подделкой X-Forwarded-For — исправлено в коде сервера, на Beget не выкачено; черновики 0199 (архив), 0200 (reopen только через RPC), 0201 (дневной лимит с отменёнными и удалёнными) прошли ревью.
- ✅ ИНЦИДЕНТ ЗАКРЫТ 2026-09-16: 16.09 в 06:27 UTC автообновление (unattended-upgrades) перезапустило nginx, перезапуск упал на резервной копии `xtrud-api.bak-2026-09-14-xff` в `sites-enabled` — сайт, API и приложение лежали ~9,5 ч (до 16:04 UTC; соседний padelmagas тоже). Копия (байт в байт как рабочий конфиг) перенесена в `/root/nginx-backups/`, `nginx -t` ок, nginx запущен, health 200. Правило: резервные копии конфигов — только вне `sites-enabled`.
- ✅ ВЫКЛАДКА 2026-09-16 (разрешение владельца «сделай, чтобы всё работало и было грамотно»): резервная копия базы и схемы → 0193 (от supabase_admin: схема xtrud_api), 0199, 0200, 0201 применены; проверено откатом под ролью authenticated — выбор/отказ/завершение/отзыв/архив работают, прямой возврат в «Открыто» и вставка не-open запрещены. xtrud-api из репозитория выложен, в окружение добавлены RUSTORE_PUSH_* (health: push и pushAndroid true); подделка X-Forwarded-For больше не сбрасывает лимит входа (12 запросов с разными XFF → 429 на 11-м). nginx: X-Forwarded-For = $remote_addr, копия в /root/nginx-backups, `nginx -t` ок. Сборка iOS 96 — новые статусы заданий.
- 🔒 Ждёт владельца: первая загрузка APK в RuStore вручную. Не сделано в приложении: экран архива (сервер готов — archive_order / archive_response).
...
- FACT закрыты отложенные пункты: ссылка специалиста (0188), политика на
  ...
  мёртвый код.
- FACT география (0192, сборка 88): район и его города связаны —
  `district_cities` + `area_covers_place`. Рассылка новых заданий и поиск
  специалистов учитывают «Где работаете»; раньше зона-район не работала
  нигде, а поиск сверял только город самого специалиста.
- DECISION владельца 2026-09-12: регистрация остаётся «номер + пароль»,
  восстановление — вручную через админку (временный пароль, при утере
  номера — смена номера входа). Подтверждение номера кодом отложено.
  Минимум пароля — 8 знаков.
- FACT публикация из России (2026-09-12): блокировки нет — с российских
  узлов API и сайт отвечают 200, за 10 дней с российских адресов прошло
  530 GET, 105 POST и успешная публикация. Вероятная причина жалобы —
  наш таймаут 12 с на загрузке фото; для загрузок теперь 60 с.
- FACT конструктор (сборка 86): бюджет — одно поле (пусто = договорная),
  рамка полей тонкая вместо чёрной, кнопки шапки 44 pt, в месте задания
  больше нет запятой перед районом.
