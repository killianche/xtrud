...
Полная ревизия продукта силами четырёх агентов (клиент / сервер / история / инфраструктура). Разведка, кода не трогали — кроме документации ниже.

**Где продукт:**
- **iOS — в App Store, продаётся** (v1.0.0, сборка 10, статус READY_FOR_SALE с 11 июня). Версия **1.0.1** с фиксом краша «тап по активному табу» помечена релизом 11 июня, но подтверждения отправки сборки 11 в Apple нигде нет — **проверить в кабинете первым делом**. Номер сборки 11 в `app.json` до сих пор не закоммичен.
- **Сайт** — `xtrud.pro` (+ `xtrud.alanbani.ru`), работает, но с **включённым demo-входом**: `DEMO_FLAG = "true"` жёстко зашит в `scripts/build-web-local.mjs`, а тот же скрипт используется для прод-выкатки. В сборках приложения demo уже выключен (`eas.json`).
- **Android / Google Play — не начинали.** Есть только имя пакета. При том что рынок в концепции описан как Android-first (~85%).
- **Пауза в работе:** с 18 июня по 28 июля ни одного коммита. Это первая сессия после перерыва.

**Размер и здоровье кода:** 266 файлов в `app/` + `src/` (~43 000 строк), 128 миграций (0001→0119), 3 edge-функции. Проверка типов чистая, есть CI (типы + линт + тесты на каждый push). Тестов 6 файлов — только чистые функции, экраны не покрыты.

**Главные находки ревизии (подробности — в разделе ниже «Ревизия 2026-07-28»):**
1. Жизненный цикл сделки полностью готов в базе (6 функций: принять отклик → работа → подтверждение → завершение) и **не подключён ни к одному экрану** — наследие разворота на «доску объявлений» 20 мая.
2. Две точки, где посторонний может получить чужие телефоны/почты (`get_master_phone`, `resolve_login_email` открыты анониму без ограничения частоты).
...
## Снимок на 2026-06-07 (предыдущий, оставлен для истории)

- **iOS — СОБРАНО И ОТПРАВЛЕНО В APP STORE (2026-06-07):** актуальная сборка
  **build 10** (v1.0.0) — **только iPhone** (`supportsTablet=false`, чтобы Apple не
  требовал iPad-скриншоты) + **новый брендовый значок** (розовый градиент + белый
  «вихрь», `assets/images/icon.png` из `logo_gradient.png`). Собрана в облаке EAS и
  загружена в App Store Connect полностью автоматически по ASC API-ключу (без 2FA).
  Тест-аккаунт ревьюера: `review.xtrud@gmail.com` / `Review2026xtrud` (пароль
  переустановлен через SQL 2026-06-07, вход 100% работает; телефон `+79280000000`).
  Тексты витрины — `APP_STORE_LISTING.md`. Осталось (владелец в кабинете): выбрать
  build 10, заполнить категорию/возраст/конфиденциальность, → Submit for Review.
  Детали и уроки — раздел «iOS-сборка» ниже. ⚠️ Push отключён в v1, splash со старым
  знаком — вернуть/обновить в апдейте.
...
## Ревизия продукта 2026-07-28 (4 агента, только чтение)

Первая сессия после паузы 18 июня → 28 июля. Полная ревизия: четыре агента параллельно разобрали клиентскую часть, базу данных, историю проекта и инфраструктуру. Всё ниже **проверено вручную** после отчётов агентов (открыты сами файлы), а не переписано с их слов.

### Что нашли — по важности

**Срочно (бьёт по людям или по запуску):**

1. **Обновление 1.0.1 возможно не отправлено.** Фикс краша «тап по активному табу» сделан 11 июня, версия в `app.json` поднята, но `buildNumber: 11` не закоммичен и записи об отправке нет. Люди, скачавшие приложение, могут до сих пор ловить вылет. → Проверить в App Store Connect.
2. **Журнал ошибок ни разу не смотрели.** Таблицу `client_errors` завели 11 июня именно ради этого. Полтора месяца данных о реальных сбоях лежат непрочитанными. Запрос — в разделе про журнал ниже.
3. **Ключ Unisender (рассылка писем) не пересоздан** — светился в переписке, от него зависит восстановление пароля. Это пункт №1 в собственном списке «до публичного запуска» с 5 июня.
4. **Demo-вход на сайте включён жёстко.** `DEMO_FLAG = "true"` прибит в `scripts/build-web-local.mjs`, и этот же скрипт выкатывает прод. Любой человек заходит на `xtrud.pro` под демо-аккаунтом, включая административный.

**Важно (архитектура и данные):**

5. **`resolve_login_email` открыта анониму** (`supabase/migrations/0117:53` — `grant … to anon`). По номеру телефона возвращает **настоящую почту** пользователя, без ограничения частоты запросов. Перебором номеров собирается таблица «телефон → почта». Проверено в файле.
6. **`get_master_phone` — подмена приватного номера публичным.** Функция открыта анониму намеренно (клиент должен звонить мастеру), но если мастер не заполнил публичный контакт, отдаётся **регистрационный** номер, который тот же файл миграции объявляет приватным (`0098:42` + `0040:35`). Плюс нет ограничения частоты — телефоны всех мастеров выгребаются перебором.
7. **`client_errors` принимает записи от кого угодно** без ограничений (`0119:31` — `to anon … with check (true)`). Таблицу можно забить мусором.
8. **Миграции не описывают реальную базу.** 9 файлов миграций — только комментарии, без кода (`0084`–`0090`, `0115`, `0116`); две функции (`get_master_stats`, `submit_master_review`) существуют только в проде и в типах. **Базу невозможно воссоздать с нуля из репозитория** — при потере проекта Supabase восстановление будет ручным.
9. **Жизненный цикл сделки готов в базе и не подключён.** Шесть функций (`accept_response` → `mark_order_done` → `confirm_work_done` → `confirm_completion` → `terminate_cooperation` + `use-orders-assigned-to-me`) написаны, хуки в коде есть, **вызовов в экранах ноль** — проверено grep по `app/`, нашёлся только комментарий в `orders/[id].tsx:973`. Это наследие разворота на «доску объявлений» 20 мая. Решение владельца: либо вычистить, либо осознанно оставить как задел.
10. **Уведомления, соцграф, бригады** — таблицы, функции и хуки есть, экранов нет. Триггеры исправно пишут строки в `notifications`, которые никто никогда не прочитает.
11. **Дубли в коде:** два `Avatar` и два `Skeleton` (в `src/components/` и `src/components/ui/`), два экрана детали кейса, `WebShell.tsx` и `CategoryTile.tsx` без единого импорта, `lucide-react-native` остался в зависимостях после переезда на Phosphor.

**Можно потом:**

12. **Android / Google Play не начинали вообще** — при том что в концепции рынок Android-first (~85%).
...
- 266 файлов в `app/` + `src/`, ~43 000 строк. Самые тяжёлые: `orders/[id].tsx` (1832), `master/[id].tsx` (1279), `category/[id].tsx` (1100).
- 128 миграций (0001→0119), из них 9 пар с одинаковыми номерами (параллельные ветки работ).
- Проверка типов — чисто. CI на каждый push (типы + линт + тесты).
- Тесты — 6 файлов, только чистые функции. Экраны и хуки не покрыты.
- Защита от разницы «браузер / телефон» дисциплинированная: все обращения к браузерным возможностям под проверкой платформы (аудит 11 июня + выборочная перепроверка сейчас).

### Новое правило работы

Крупная задача теперь начинается с подключения агентов-помощников — [`.claude/rules/agent-delegation.md`](.claude/rules/agent-delegation.md). Триггеры, каталог агентов, разбиение на непересекающиеся зоны, шаблон брифа, обязательная проверка находок перед правкой кода. Запрошено владельцем 2026-07-28. Обкатано на этой самой ревизии.
...
## Авторизация по паролю (2026-06-05) — ✅ сделано, проверено вживую

Замена SMS-OTP на пароль + почта (SMS.ru требует платные буквенные имена отправителя 2000–2550 ₽/мес). Решение владельца. Что сделано:
- **Логика** (`src/lib/auth.ts`): `registerWithCredentials` (через серверную функцию `register-user`), `loginWithCredentials` (почта/телефон → RPC `resolve_login_email` → signInWithPassword), `requestPasswordReset` (resetPasswordForEmail → `xtrud.pro/reset-password`), `updatePassword`. Старые `sendOtpToPhone`/`verifyOtpCode` — дормант.
...
Полная проверка цепочки вход/регистрация/сброс/письма. Главное:
- **Вход по номеру в любом формате — РАБОТАЕТ.** RPC `resolve_login_email` сводит
  телефон к последним 10 цифрам, чистит `+ ( ) - пробелы`, отбрасывает ведущую
  `7`/`8`. Проверено вживую для `+79289204029`, `79289204029`, `89289204029`,
  `9289204029` и маски — все находят один аккаунт владельца. Жалоба «не всегда
  срабатывает» — вероятно неверный пароль легаси-аккаунта владельца (`@ingush`,
  email `79289204029@xtrud-demo.local`, не настоящая регистрация) ИЛИ старая
  клавиатура в поле логина (см. фикс 3).
- **Баг 1 (важно): foreign-номера искажались при регистрации.** `normalizePhone`
  был РФ-only и переписывал любой собранный `+49…`/`+1…`/`+375…` в кривой `+7…`
  (терялись цифры). Починено: `normalizePhone` (validation.ts) теперь пропускает
  уже собранный международный номер без искажения, РФ/КЗ приводит к +7XXXXXXXXXX.
- **Баг 2 (важно): «осиротевшие» аккаунты при занятом номере.** `register-user`
  создавал auth-пользователя, а сохранение телефона при конфликте UNIQUE молча
  глоталось → аккаунт без телефона, вход по номеру невозможен. Починено: проверка
  занятости номера ДО создания + откат (deleteUser) при гонке. **Нужен редеплой
  функции `register-user`** (правка в репо, в проде ещё старая v1).
- **Баг 3 (мелочь): клавиатура поля входа.** Было `inputMode="email"` на поле
  «почта ИЛИ телефон» — прятала цифры. Стало `inputMode="text"`.
- Исторические 504-таймауты в логах /recover — это СТАРЫЙ путь
  `resetPasswordForEmail`; новые запросы идут через `send-reset-email`
  (`generate_link`), таймаутов больше нет.
- **`register-user` редеплоен (v2)** — баги 1+2 в проде закрыты. Аудит-фиксы (3
  бага) выкачены на сайт.

### Батч багов профиля + гостевого заказа (2026-06-06) — ✅ выкачено

Владелец нашёл серию багов на экранах профиля/смены номера. Аудит-агент + ручная
доработка:
- **ChangePhoneSheet** (`src/features/profile/`): убран мёртвый SMS-flow (вызывал
  «Invalid payload sent to hook») → одношаговое прямое сохранение номера без OTP +
  проверка занятости (`use-user-private` отдаёт человекочитаемую ошибку) + видимая
  кнопка (`bg-primary`+`text-on-primary` вместо тёмного текста на тёмном).
- **edit-client / edit-master**: «Отмена» теперь работает на web (`Alert.alert` с
  кнопками на RN-web — no-op → web-safe confirm); ошибки сохранения видны строкой;
  «Сохранить» больше не блокируется пустым/невалидным юзернеймом при смене только
  имени.
- **profile/index**: смена фото на web починена; **settings**: убран текст про SMS.
- **JitSignupSheet** (гостевая публикация заказа, `/orders/new`): был тот же мёртвый
  SMS → одношаговый анонимный вход (`signInAnonymouslyWithPhone`, без SMS/пароля,
  низкий порог) + видимая кнопка. Анонимный вход в Supabase включён (проверено).
- **Тестовый аккаунт** `pmrhhm@gmail.com` / `+79288234994` — создан при отладке
  сброса; владельца залогинило в него recovery-сессией. Кандидат на удаление.

---

## Батч профиль/онбординг/заказы (2026-05-29) — ✅ завершён и выкачен

Серия правок по фидбэку владельца (19 задач, #149–#167) — все закрыты, в проде:
- **Логин:** галочка согласия отмечена по умолчанию (`acceptedTerms=useState(true)`,
  `phone.tsx`). ⚠️ перед App-Store-сабмитом вернуть false (5.1.1 + 152-ФЗ).
- **Убрана фамилия — везде только имя:** поле «Фамилия» убрано из
  `MasterProfileFormBody` (онбординг + edit-master) и `edit-client.tsx`; схема
  `master-profile-schema` → `lastName: z.string()` (пустое проходит);
  `UpdateMyProfileInput.last_name` опционально; подпись «Имя и фамилия» → «Ваше имя».
  Данные: `UPDATE users SET last_name=NULL` (демо). Логика отображения
  `[first,last].filter(Boolean).join` сама показывает только имя.
- **Фикс «фото заказа не сохраняются»** (`src/lib/image-upload.ts`): на сайте
  выбор файла через `<input type=file>` имел гонку — «отмена по возврату фокуса»
  делала `settle([])` через 500мс, и если большое фото с телефона декодировалось
  дольше — выбор терялся, заказ/аватар сохранялся без фото (хранилище
  `order-photos` и `avatars` имели 0 файлов за всё время). Добавлен флаг
  `changeStarted`: focus-fallback считает отменой только если выбор так и не
  начался. Подтверждено в браузере (медленное чтение 1500мс теперь не теряет
  фото) и тестом загрузки в Supabase Storage (права/RLS целы). Касается фото
  заказов, аватаров и портфолио на вебе. ⚠️ видно у пользователя только после
  выкатки сайта.
- **Срок заказа «К дате» (точная дата через календарь)** (роль xtrud-designer):
  в блоке «Сроки» формы заказа добавлен 5-й вариант «К дате» — chip с иконкой
  календаря (Phosphor `CalendarBlank`). Тап открывает новый bottom-sheet-календарь
  `src/features/orders/OrderDateSheet.tsx` (чистый RN, без сторонних date-picker:
  месячная сетка Пн–Вс, прошлые дни приглушены и не нажимаемы, сегодня = синее
  кольцо, выбранный день = синяя заливка, быстрые пилюли «Сегодня»/«Завтра»,
  primary «Готово»). Выбор даты ставит `urgency="by_date"` + `preferred_date`;
  выбор любого из 4 относительных сроков обнуляет дату (mutex §E). Chip с
  выбранной датой показывает «К 12 июня». Проброшено в `OrderFormBody` (новые
  пропсы `preferredDate`/`setPreferredDate`), подключено в `new.tsx` и
  `edit/[id].tsx` через `watch`/`setValue`. Слой данных (enum `by_date`,
  колонка `preferred_date`, схема, хелперы) был готов заранее — не трогался.
  Проверено в обеих темах в браузере; tsc и 8 grep-чеков чистые.
- **Срок «К дате» в заказе** (#157): в блоке «Сроки» 5-й вариант «К дате» +
  календарь `OrderDateSheet` (кросс-платформенный, обе темы). База: enum
  `order_urgency += by_date`, колонка `orders.preferred_date`. Показ во всех
  лентах и на странице заказа (`formatOrderTiming` → «К 12 июня»). Сделано через
  роль дизайнера.
- **Редизайн окон выбора** (#160): общий `PickerSheet` и переключатель
  готовности мастера (`AvailabilitySwitcher`) — по жалобе «галочки/выделение/
  стрелка назад некрасивые». Новый индикатор «выбрано» — круглый `SelectedMark`
  (bg-primary + Check в on-primary, контраст §A в обеих темах), мягкая подсветка
  выбранной строки (bg-canvas-soft, инсет-rounded), серая круглая стрелка-назад
  заменена на лёгкий крестик «Закрыть». Проверено: tsc + 8 grep-чеков чистые,
  приложение рендерится.
- **Редизайн страниц редактирования профиля** (#161): `edit-master` приведён к
  единому виду с `edit-client` — навбар «Отмена / Профиль мастера / Сохранить»
  сверху (Save закреплён и всегда доступен; раньше был `ScreenHeader` + кнопка
  «Сохранить» в самом низу длинной формы, уезжала при прокрутке), фон
  `bg-canvas-soft` (поля-карточки на нём как у клиента). `edit-client` уже был в
  этом паттерне — не трогал. Эталон Lazyweb: Bluesky/Lawfully (Cancel/Save в
  навбаре, disabled-until-dirty). tsc + grep-чеки чистые.
- **«Хочу стать мастером»** (#165): большая тёмная кнопка во всю ширину под
  аватаром → тихая ghost-ссылка (иконка `Wrench` + текст в `text-mute`) ниже,
  под карточкой «Редактировать профиль». Меньше, незаметнее, не ловит случайные
  тапы (фидбэк владельца).
- **Адаптивный фавикон** (#166): `public/favicon.svg` — только логотип xtrud на
  прозрачном фоне, цвет через `prefers-color-scheme` (тёмный `#111827` в светлой
  теме, белый в тёмной). Ссылка инжектится в `index.html` сборкой
  (`scripts/build-web-local.mjs`), т.к. Expo static-render выкидывает кастомный
  `rel="icon"` из `+html.tsx` и ставит свой `/favicon.ico` (остаётся фолбэком).
  Проверено в превью: `/favicon.svg` отдаётся 200, обе ссылки в HTML.
- **Текст про номер клиента** (#167): в `edit-client` фраза «Мастера не видят
  ваш номер до того, как вы сами это разрешите» (намекала, что номер можно
  открыть) → «Ваш номер всегда скрыт от мастеров. Вы сами звоните тем, кто
  откликнулся.» Соответствует модели: клиентский номер ВСЕГДА скрыт, клиент
  сам связывается. Текст при создании заказа уже был корректным (#158).
- **Мастер обжалует отзыв** (#163): мастер на своей странице (`/master/[id]`,
  `isOwnProfile`) видит оставленные отзывы и жмёт «Обжаловать» → новое окно
  `ReportReviewSheet` (причина + комментарий) → запись в `reports`
  (`target_type='review'`) через существующий `useCreateReport`. Модератор в
  `/admin/reports` уже умеет «Скрыть отзыв» (`status='hidden'` → пропадает, т.к.
  `useReviewsForTarget` фильтрует `status='visible'`). Бэкенд жалоб и админ-часть
  существовали — добавлена только точка входа мастера: кнопка «Обжаловать» в
  `ReviewsSection` (через опц. `onReport`) + карточка «Мои отзывы» в профиле
  мастера. Заодно убран inline-hex звёзд в `ReviewsSection` (§B). tsc + 8
  grep-чеков чистые; страница мастера рендерится, кнопка скрыта у не-владельца.
- **Иконки категорий — свои вместо неподходящих эмодзи** (2026-05-29): для 8
  строительных категорий эмодзи не соответствовали смыслу (гипсокартон=кисть,
  плитка=флаг, потолки=лампа, натяжные=плёнка, отопление=градусник, утепление=
  шарф, заборы=замок, скважины=кран). Нарисованы свои flat-color SVG-иконки
  (лист ГКЛ / сетка плиток / светильник / глянец-споты / радиатор / рулон ваты
  / штакетник / капля+грунт) → `LOCAL_CATEGORY_ICON_URI` (приоритет над CDN).
  Исходники `assets/icons/category/*.svg`, генератор `scripts/gen-category-icons.mjs`.
  Проверено в браузере на экране выбора категории — все совпадают.
- _(остальные задачи батча дописываются по мере выполнения)_

## Личный аккаунт владельца: Руслан @ingush (2026-05-28)

- Аккаунт на номере владельца **+7 928 920-40-29**: имя **Руслан**, юзернейм
  **@ingush**, роль клиент, город Магас. На номере уже был анонимный аккаунт
  «Хамзат» — перенастроен под владельца (миграция 0113), второй не плодили.
- **Вход (пока нет SMS):** номер +79289204029 добавлен в `DEMO_EXTRA_PHONES`
  (`src/lib/auth.ts`) → demo-вход по email `79289204029@xtrud-demo.local` + пароль
  `xtrud`, код из SMS — любые 6 цифр. Работает только при `EXPO_PUBLIC_ENABLE_DEMO=true`.
- Мастером можно стать в приложении (юзернейм повторно не спрашивается — guard в
  `master-profile.tsx`: поле скрыто, если username уже есть).

## Система юзернеймов (@ingush) при регистрации (2026-05-28)

Решение владельца: при регистрации человек создаёт уникальный юзернейм (как в
Instagram), закреплённый за ним. Стабильная личность независимо от номера —
помогает при смене номера + ручном восстановлении через поддержку.

- **База (миграция 0112):** колонка `users.username` (nullable, CHECK формата
  `^[a-z0-9_.]{3,30}$`, регистронезависимый unique-индекс). RPC
  `is_username_available(p_username)` — живая проверка; `set_username(p_username)`
  — закрепляет один раз (если уже есть — `username_already_set`, менять нельзя;
  восстановление — поддержкой под service_role). Обе REVOKE от anon, GRANT
  authenticated.
- **Хуки/компонент:** `src/features/auth/use-username.ts` (normalize, format-check,
  `useUsernameAvailability` с debounce 400мс, `useSetUsername` + маппинг ошибок) +
  `src/features/auth/UsernameField.tsx` (поле с @, живой статус свободно/занято,
  токены + Phosphor, без inline-hex).
- **Флоу регистрации:** поле добавлено в `client-name.tsx` (клиент) и
  `master-profile.tsx` (мастер, шаг 1/3). Юзернейм обязателен — кнопка
  «Продолжить» активна только когда имя ок И юзернейм свободен. Сначала
  `set_username`, потом завершение онбординга.
- **Показ:** `@username` под именем на своём профиле (`profile/index.tsx`).
- **Существующие аккаунты:** без юзернейма (колонка nullable) — для них необязателен.

## Лимит откликов: отзыв возвращает слот + честные тексты заказа (2026-05-28)

- **Лимит (миграция 0111):** `get_response_limit_today` и триггер
  `check_daily_response_limit` теперь считают отклики за день с фильтром
  `status <> 'withdrawn'`. Раньше отозванный отклик «съедал» дневной слот (5/день)
  — мастер неожиданно терял попытку. Отклонённый клиентом (rejected) по-прежнему
  считается. `use-withdraw-response.ts` инвалидирует `RESPONSE_LIMIT_QUERY_KEY` —
  освободившийся слот виден сразу.
- **Ложные тексты (`orders/[id].tsx`):** «Клиент уже выбрал мастера…» → «Заказ
  закрыт — отклики больше не принимаются»; статус отклика accepted «Клиент выбрал
  вас» → «Отклик активен». В модели доски объявлений клиент не выбирает мастера
  в приложении.
- **Рейтинг в карточке отклика:** `useOrderResponses` подтягивает
  `master_profiles.rating_overall_avg/count`; в карточке отклика клиента под
  именем мастера — ★ + рейтинг + (кол-во), скрыт при 0 отзывов.
- **Честные тексты вместо обещания времени:** заказ без откликов >24ч →
  «Пока никто не откликнулся» + совет (дополнить описание/фото/бюджет); свежий →
  «Уведомим, как только мастер отзовётся» (без «в течение часа»). Экран после
  публикации: убрано «15–60 минут». Пустой список заказов: убрано «в течение часа».

## Подключено отслеживание сбоев (Sentry) (2026-05-28)

Пункт 6 предзапускового аудита. Чтобы видеть реальные падения/ошибки у
пользователей, а не узнавать о них из жалоб.

- **Установлен** `@sentry/react-native@~7.2.0` (версия под Expo SDK 54;
  ставили с `--legacy-peer-deps` — у проекта есть конфликт lottie, не связан).
- **Обёртка** `src/lib/sentry.ts`: `initSentry()` + `reportError()`. Включается
  **только** при наличии `EXPO_PUBLIC_SENTRY_DSN`. Без ключа — тихий no-op,
  приложение работает как обычно. В dev события на сервер не шлются (`enabled:
  !__DEV__`), только debug-лог.
- **Запуск** в `app/_layout.tsx` (вызов `initSentry()` до рендера).
- **Обработчик ошибок** `AppErrorBoundary` теперь шлёт ошибку через `reportError`
  (раньше был TODO).
- **Нативные крэши** iOS/Android: добавлен config-plugin `@sentry/react-native/expo`
  в `app.json`. Для расшифровки отчётов (source maps) при сборке нужны
  organization/project + `SENTRY_AUTH_TOKEN` — пока не заданы (плагин это лишь
  предупреждает, сборку НЕ ломает). На web достаточно одного DSN.
- **env:** `EXPO_PUBLIC_SENTRY_DSN` добавлен в `env.ts` (optional) и в `.env.example`.
- **Проверено:** веб-сборка (`expo export`) с новой зависимостью и плагином
  собирается и грузится без ошибок (без DSN — Sentry выключен).
- **❗ Нужно от владельца, чтобы включить:** создать бесплатный проект на
  sentry.io → дать DSN → положить в `.env.local` + EAS/CI secrets → пересобрать.

## Удалена переписка между пользователями (чат) — полностью (2026-05-28)

Решение владельца: «чат, переписка между двумя людьми нам не нужна». Экраны
чата убраны ещё 2026-05-20 (модель доски объявлений — клиент звонит/пишет
мастеру напрямую). Эта сессия добивает остатки в базе и текстах.

- **База (миграция `0110_remove_chat_messaging`):** удалены таблицы `chats`
  (9 демо-чатов) и `messages` (45 демо-сообщений) каскадом + их триггеры; удалены
  функции `start_chat_with_master`, `mark_chat_read` и 4 триггер-функции
  сообщений. Из `accept_response` убрано создание чата (остальная логика цела;
  функция всё равно в приложении не вызывается). Проверено: на chats/messages
  не ссылается ни один внешний ключ — каскад безопасен.
- **Хранилище:** у бакета `chat-images` сняты все правила доступа и снят
  публичный флаг (`public=false`) → фото из чата недоступно никому. Прямое
  удаление файла блокирует защита Supabase; пустой бакет можно удалить вручную
  в панели. Это закрывает вопрос приватности фото из чатов.
- **Типы:** из `src/types/database.ts` удалены таблицы chats/messages и
  функции start_chat_with_master/mark_chat_read.
- **Тексты (важно для App Store):** `app/legal/privacy.tsx` и `terms.tsx` больше
  не описывают «сообщения в чате», «синхронизацию чатов», «открывается чат» —
  тексты приведены к реальности (прямой контакт по телефону/WhatsApp).
- **Комментарии:** поправлены устаревшие ссылки на /chats в `use-withdraw-response.ts`,
  `ScreenHeader.tsx`, `master/[id].tsx`.
- **Оставлено:** «Сообщение клиенту» в карточке заказа — это текст ОТКЛИКА
  (сопроводительное к отклику), не чат. Хук `use-accept-response.ts` оставлен
  (мёртвый, но рабочий — модель «выбранного мастера» — отдельный вопрос).
