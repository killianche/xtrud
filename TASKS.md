...
- [ ] 🚧 **Улучшить category grid + убрать не-ремонтные категории**
  - Запрос пользователя: «У нас мастера по ремонту. Убери маникюры, грузоперевозки, школьные предметы, кейтеринг, брови — только ремонт + стройка»
  - Шаги:
    - Lazyweb research за category grid в repair marketplaces (был запрос — MCP отвалился, повторить когда вернётся)
    - SQL: посмотреть полный список `categories_l2` на проде
    - SQL: `UPDATE categories_l2 SET is_visible = false WHERE id IN (...)` для не-ремонтных
    - UI: улучшить плитку (фото обложки + overlay ИЛИ Lucide-иконка + цвет)
    - Регенерировать types + проверить
  - Блокер: ждать переподключения Lazyweb MCP (можно работать без него — категории очевидные)
