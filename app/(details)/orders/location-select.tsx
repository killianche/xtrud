            ...
            <>
              ...
              {citiesSource === "bundle" ? (
                <View
                  ...
                  className="mt-3 rounded-md border border-hairline bg-canvas-soft px-3 py-2"
                >
                  <AppText className="text-caption text-muted">
                    Используется сохранённый список городов. Перед публикацией выбранный город будет
                    проверен.
                  </AppText>
                </View>
              ) : null}
              {citiesError && cities === undefined ? (
                <View accessibilityLiveRegion="polite" className="items-start py-4">
                  <AppText accessibilityRole="alert" className="text-body-sm text-muted">
                    Не удалось загрузить города. Район или «Вся Ингушетия» по-прежнему доступны.
                  </AppText>
                  <Pressable
                    accessibilityRole="button"
                    accessibilityLabel="Повторить загрузку городов"
                    disabled={citiesRefetching}
                    onPress={() => void refetchCities()}
                    className="mt-3 min-h-11 justify-center rounded-full border border-hairline px-5 active:bg-canvas-soft disabled:opacity-50"
                  >
                    <AppText weight="semibold" className="text-body-sm text-ink">
                      {citiesRefetching ? "Загружаем…" : "Повторить"}
                    </AppText>
                  </Pressable>
                </View>
              ) : cities === undefined ? (
