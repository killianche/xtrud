...
export default function EditMasterScreen() {
  ...
  const hasUnsavedChanges = isDirty || usernameChanged;
  ...
  // One removal policy for the visible action and the native iOS edge-swipe.
    ...
    >
      ...
      <View className="flex-row items-center justify-between border-hairline border-b bg-canvas px-4 py-3">
        <Pressable
          ...
          onPress={onCancel}
          hitSlop={12}
          className={`min-w-[64px] active:opacity-60 ${isBusy ? "opacity-40" : ""}`}
        >
