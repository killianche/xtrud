...
| 8 | Privacy manifest `PrivacyInfo.xcprivacy` с required-reason API и собранными причинами зависимостей | [Privacy manifest files](https://developer.apple.com/documentation/bundleresources/privacy-manifest-files), [Expo privacy manifests](https://docs.expo.dev/guides/apple-privacy/) | **FACT**: ключ `ios.privacyManifests` в `app.json` отсутствует. Expo-пакеты несут свои `PrivacyInfo.xcprivacy`, но Apple «does not correctly parse all the PrivacyInfo files included by static CocoaPods dependencies» — агрегация на нас. Достаточность текущей конфигурации — **UNKNOWN**, проверяется ответом ASC на upload |
