...
| Шторка фильтров: строка «Сбросить · Фильтры · Готово», inset-группы, выбор галочкой, применяется сразу, без кнопки «Применить» | Фильтры App Store / Карт / Почты | `FilterSheetPage`, `InsetRow checked` |
