...
- DECISION владельца 2026-09-06: репозиторий можно сделать публичным — тогда
  стандартные macOS-раннеры бесплатны без лимита. Переключение делает владелец
  в GitHub (Settings → General → Danger Zone → Change visibility): у токена,
  выданного агенту, нет права Administration.
