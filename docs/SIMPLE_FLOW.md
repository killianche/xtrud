...
> ⚠️ **Обновление 2026-05-28:** изначально (2026-05-20) БД не трогали — только UI.
> Но позже по решению владельца часть удалена и на уровне базы:
> - **Оспаривание (dispute)** — RPC `open_dispute` удалён (миграция 0109).
> - **Чат / сообщения** — таблица сообщений + связанные политики удалены (миграция 0110).
>
> Поэтому **простой `git revert` UI больше НЕ восстановит чат/спор полностью** —
> понадобится и откат этих миграций. Остальное (lifecycle-RPC, enum статусов)
> по-прежнему в БД.

Что осталось в БД как было:
