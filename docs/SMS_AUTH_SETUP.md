...
> ⚠️ **НЕАКТУАЛЬНО / ДОРМАНТ (с 2026-06-05).** SMS-вход заменён на **телефон/почта
> + пароль**, потому что SMS.ru требует платные буквенные имена отправителя
> (2000–2550 ₽/мес за каждое). Актуальная модель входа — CLAUDE.md инфра-правило
> №6 + STATUS.md «Авторизация по паролю». Код SMS (`send-sms`, `sendOtpToPhone`,
> экран `verify.tsx`) оставлен **дормантом** на случай возврата SMS в будущем —
> этот документ описывает, как его снова включить. Сейчас выполнять его шаги НЕ нужно.

---

> TL;DR (историческое): код приложения уже готов (реальный OTP-вход), edge-функция `send-sms`
