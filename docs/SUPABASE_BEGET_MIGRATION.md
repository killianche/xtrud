...
> **Статус на 2026-09-06: перенос выполнен.** Приложение и все серверные
> функции работают на Beget (`api.xtrud.pro`); последняя ссылка на облачный
> проект (`notify_user`) убрана миграцией 0165. Ночная копия базы и Storage —
> `/opt/xtrud/backup.sh` (cron 04:40, 14 дней). Текст ниже — история плана,
> его gate-и относятся к периоду до 2026-08-31.
