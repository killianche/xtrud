...
> 🛑 **LEGACY — ФУНКЦИЯ УДАЛЕНА (2026-05-20 UI, миграция 0110 — БД-сообщения).**
> Чат / переписка внутри приложения **удалены** при переходе на classifieds-модель.
> Сейчас контакт клиент↔мастер — только звонок и WhatsApp (номер клиента всегда
> скрыт, инициатор — клиент). Этот документ оставлен **только** на случай возврата
> чата. Текущая модель — CLAUDE.md «Актуальная модель продукта» + `docs/SIMPLE_FLOW.md`.
