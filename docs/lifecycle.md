...
> 🛑 **LEGACY — БОЛЬШАЯ ЧАСТЬ НЕ АКТУАЛЬНА (2026-05-20).** Этот документ описывает
> полный lifecycle сделки (выбор мастера, «в работе», подтверждение выполнения,
> спор, чат). При переходе на **classifieds-модель** всё это **удалено**: нет
> чата, нет выбора мастера, нет статусов «в работе/awaiting/disputed». Заказ
> живёт в open / cancelled / expired (+ draft); мастер откликается, клиент сам
> звонит/пишет в WhatsApp. RPC (`accept_response` и т.п.) в БД остались, но в UI
> не вызываются. **Актуальная модель** — CLAUDE.md «Актуальная модель продукта»
> + `docs/SIMPLE_FLOW.md`. Этот файл — историческая справка / план на случай
> возврата lifecycle.

> ⚠️ **УДАЛЕНО 2026-05-28 — функционал «Оспорить» (спор).** По решению владельца
> продукта спор полностью убран: RPC `open_dispute` удалён из базы, хук
> `use-open-dispute.ts` удалён, видимой кнопки в UI не было. Все переходы и
> упоминания ниже, связанные со спором — **T10, T12, T14, статус `disputed`,
> поля `dispute_*`** — больше НЕ актуальны. Сам статус `disputed` и поля
> `dispute_opened_by/dispute_reason/disputed_at` оставлены в базе как безвредные
> неиспользуемые остатки (0 записей) — их трогает только журнал смены статусов.
> **Не реализовывать спор заново без явного запроса владельца.** Путь «работа не
> доведена до конца» закрывается через `terminate_cooperation` (T6t).
