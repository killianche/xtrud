...
## 🚨 ПРАВИЛО: Назрань + Магас = ОДИН город «Назрань · Магас» (везде)

**Решение владельца 2026-05-24.** Назрань и Магас — единая агломерация, фактически один город. Поэтому **во всех списках выбора города и фильтрах** они показываются **одной плашкой «Назрань · Магас»** (id `nazran-magas`), и **никогда раздельно**.

- Раздельные id `nazran` / `magas` остаются в таблице `supabase.cities` (и в `MAJOR_CITIES` с флагом `hiddenInPicker:true`) **только для legacy-lookup** имён старых записей — но из любых списков выбора они скрыты.
- Источник истины скрытия — `HIDDEN_PICKER_CITY_IDS` в [`src/lib/location-config.ts`](../src/lib/location-config.ts). Config-списки (`PICKER_CITIES`) уже фильтруют их; **БД-список** `supabase.cities` фильтруется в [`useCities`](../src/features/cities/use-cities.ts) тем же набором — поэтому экран выбора локации заказа (`<LocationPicker>`) тоже показывает только объединённый вариант.
- Заказ с `city_id='nazran-magas'` виден мастерам Назрани И Магаса (`PAIR_CITIES`).
- При добавлении нового парного/объединённого населённого пункта — тем же паттерном: объединённая запись в `MAJOR_CITIES` + раздельные с `hiddenInPicker:true` + добавить их в скрытие.

**Никогда** не рендерить отдельные «Назрань» и «Магас» как выбираемые города. Если увидел их раздельно в каком-то списке — этот список берёт города в обход `PICKER_CITIES` / `useCities`-фильтра, надо починить.
