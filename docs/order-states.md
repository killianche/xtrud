...
> 🛑 **LEGACY DOC — НЕ АКТУАЛЬНО.** Описывает старый order state-machine (6→8 статусов, T1–T15, `disputed`, `awaiting_confirmation`). При переходе на **classifieds-модель** (2026-05-20) lifecycle сделки удалён: заказ живёт в `open` / `cancelled` / `expired` (+ `draft`), без «в работе» / «подтверждения» / «спора». **Актуальная модель** — CLAUDE.md «Актуальная модель продукта» + [`SIMPLE_FLOW.md`](./SIMPLE_FLOW.md). `lifecycle.md` — тоже legacy. Этот файл оставлен только для исторической справки.
