# Дизайн-референс: Cal.com

> Документ-референс. Полное описание дизайн-системы Cal.com — взято как образец визуального языка.
> Зачем здесь: модерн-SaaS эстетика (белый холст + чёрные CTA + кастомный display-шрифт + продуктовые фрагменты UI прямо в маркетинге) — кандидат на основу визуальной идентичности нашего проекта.
> **Решение применять/не применять — отдельный разговор. См. примечание в конце.**
...
**Направление зафиксировано:** чистый современный SaaS-дизайн в духе Cal.com. Никаких этнических, национальных или "тёплых региональных" визуальных мотивов. Цель — продукт читается как профессиональный западный сервис, а не как локальное приложение.

### Что берём из Cal.com (полностью)
1. ✅ **Холодный монохром** — #111 primary, #fff canvas, #f5f5f5 карточки. Никакой "тёплой брендовой краски".
2. ✅ **Чёрные CTA на белом** — единая action-color по всему продукту.
3. ✅ **Дисциплина типографики** — display + body, чёткое разделение, ничего не размывать.
4. ✅ **Сетка отступов** 4/8/12/16/24/32/48/96 dp.
5. ✅ **Иерархия радиусов** — 8 кнопки / 12 карточки / 16 hero / pill теги / full аватары.
6. ✅ **Продуктовые UI-фрагменты в маркетинге** — реальные карточки мастеров, реальные формы заказа.
7. ✅ **Аватары-кружки 36 dp** с пастельной заливкой как фолбэк для имени без фото.
8. ✅ **Тёмный футер** как визуальный финал каждой страницы (web + landing).
9. ✅ **Hairline borders** (#e5e7eb 1px) вместо тяжёлых теней.
10. ✅ **Subtle drop shadows** (`0 1px 2px rgba(0,0,0,0.05)`, `0 4px 12px rgba(0,0,0,0.08)`) — soft elevation.
11. ✅ **Сдержанные badge-пастели** (orange/pink/violet/emerald) — только для фолбэков аватаров и тегов категорий, никогда на CTA.

### Что заменяем (вынужденно)
- **Cal Sans → Inter** (или Manrope) weight 600 с letter-spacing -0.04em на display. Cal Sans лицензирован, использовать нельзя.
- **Section padding 96 dp → адаптивная шкала** 64 dp на web / 48 dp на mobile. Для mobile-first проекта 96 везде сделает экраны пустыми. На web сохраняем 96.

### Открытые решения (определяемся вместе)
- ❓ **Акцентный цвет** для редких ссылок и customer-story бейджа: оставляем Cal.com-овский #3b82f6 (blue), берём другой нейтральный (slate, zinc), или вообще убираем — полный монохром?
- ❓ **Тон голоса в копирайте** — Cal.com пишет короткими английскими SaaS-фразами ("The better way to schedule..."). На русском копируем тот же лаконичный стиль или нужен свой?
- ❓ **Стиль иконографики** — Lucide (тонкие линии, нейтральные, подходят под Cal.com эстетику) подтверждаем как стандарт?
- ❓ **Light + dark theme** или только light на старте?

### Параллельные референсы того же визуального направления
Чтобы расширить вокабуляр и не быть привязанным к одному источнику, имеет смысл изучить:
- **Linear** (linear.app) — самый дисциплинированный современный SaaS, тёмная и светлая темы, минимализм.
- **Vercel** (vercel.com) — белый/чёрный, hairline, очень похожий tone of voice.
- **Stripe** (stripe.com) — чуть более тёплый монохром, но та же школа.
- **Notion** (notion.so) — тот же подход к карточкам и spacing.
- **Raycast** (raycast.com) — продуктовые UI-фрагменты как маркетинг, идеальный референс этого подхода.
- **Arc Browser** (arc.net) — современный SaaS-минимализм с лёгкими градиентами (можно подсмотреть как делать "не скучно" в монохроме).

**Предложение:** хочешь — разберу 1-2 из этих в таком же формате (например Linear или Vercel), чтобы у нас было 2-3 источника одного направления, а не один. Это поможет позже формулировать собственные правила, опираясь на консенсус, а не на одну компанию.
