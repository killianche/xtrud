...
//
// Dynamic Type: до 2026-08-30 здесь был глобальный `maxFontSizeMultiplier={1.3}`,
// который обрезал масштаб ~130% на ВСЁМ тексте приложения, включая описания
// заданий, отзывы и заголовки — при том, что iOS даёт до 300%+ на AX5
// (`docs/IOS_FOUNDATION.md` §3.4, §9.2 п.3). Причина капа нигде не
// зафиксирована (ни здесь, ни в истории коммитов) — похоже на защитный дефолт
// «на всякий случай», а не решение под конкретную вёрстку. Кап снят: масштаб
// по умолчанию не ограничен. Если конкретному месту (бейдж/чип/счётчик/
// однострочная метрика) кап действительно нужен — он задаётся точечно через
// проп `maxFontSizeMultiplier` у вызова, с комментарием почему.
