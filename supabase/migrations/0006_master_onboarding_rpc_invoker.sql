-- Migration 0006 — переключаем complete_master_onboarding с SECURITY DEFINER на INVOKER.
--
-- Причина: advisor lint authenticated_security_definer_function_executable.
-- SECURITY DEFINER не нужен: RLS на public.users и public.master_profiles
-- уже разрешает auth.uid() = id и auth.uid() = user_id соответственно.
-- Функция в plpgsql сама по себе атомарна (BEGIN..END = транзакция),
-- так что мы получаем тот же эффект без обхода RLS.
...
-- GRANTS повторяем (CREATE OR REPLACE их не сохраняет, на всякий случай)
